    ...
    [thisObj](FHttpRequestPtr HttpRequestFulfilled, FHttpResponsePtr HttpResponse, bool bSucceeded) mutable
        {
            ...
            {
                thisObj.Get()->ResponseFailed(HttpRequestFulfilled->GetStatus());
            }
        }
    );
...
void UChatGptApiClient::ResponseFailed(EHttpRequestStatus::Type Status)
{
    switch (Status)
    {
...
void UChatGptApiClient::ProcessResponse_Implementation(const FString& ResponseContent)
{
    ...
    if (FJsonSerializer::Deserialize(JsonReader, JsonObject) && JsonObject.IsValid())
    {
        ...
        auto UsageObject = JsonObject->GetObjectField("usage");
        ...
        auto Message = Choice0->GetObjectField("message");
