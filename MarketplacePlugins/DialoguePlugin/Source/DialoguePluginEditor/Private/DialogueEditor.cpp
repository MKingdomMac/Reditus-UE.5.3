...
public:
	...
	{
		if (FSlateApplication::IsInitialized() && DialogueEditorPtr.IsValid() && DialogueEditorPtr.Pin()->GetDialogueBeingEdited()->bRefreshDetails)
		{
			TArray<UObject*> SelectedObjects;
			SelectedObjects.Add(GetObjectToObserve());
			...
			DialogueEditorPtr.Pin()->GetDialogueBeingEdited()->bRefreshDetails = false;
		}
	}
};
