...
#define IMAGE_PLUGIN_BRUSH( RelativePath, ... ) FSlateImageBrush( FPixel2DMapStyle::InContent( RelativePath, ".png" ), __VA_ARGS__ )
#define IMAGE_BRUSH(RelativePath, ...) FSlateImageBrush(StyleSet->RootToContentDir(RelativePath, TEXT(".png")), __VA_ARGS__)
#define BOX_BRUSH(RelativePath, ...) FSlateBoxBrush(StyleSet->RootToContentDir(RelativePath, TEXT(".png")), __VA_ARGS__)
#define DEFAULT_FONT(...) FCoreStyle::GetDefaultFontStyle(__VA_ARGS__)

FString FPixel2DMapStyle::InContent(const FString& RelativePath, const ANSICHAR* Extension)
{
	static FString ContentDir = IPluginManager::Get().FindPlugin(TEXT("Pixel2D"))->GetContentDir();
	return (ContentDir / RelativePath) + Extension;
}

TSharedPtr< FSlateStyleSet > FPixel2DMapStyle::StyleSet = nullptr;
TSharedPtr< class ISlateStyle > FPixel2DMapStyle::Get() { return StyleSet; }

FName FPixel2DMapStyle::GetStyleSetName()
{
	static FName PixelStyleName(TEXT("Pixel2DMapStyle"));
	return PixelStyleName;
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION

void FPixel2DMapStyle::Initialize()
{
	// Const icon sizes
...
void FPixel2DMapStyle::Shutdown()
{
	if (StyleSet.IsValid())
	{
